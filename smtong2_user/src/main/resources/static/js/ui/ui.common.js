...
// Card Expand
function cardExpand(element) {
	var par = $(element).parent('.status-gift');
	par.toggleClass('is-expanded');
...
// Card Expand
function cardExpand2(id, button) {
...
// Select Valued
function setSelectValued(buttonId, option, selected, callback) {
...
// 원화 Input 정규식
function setWonRegularNumber(input){
	
	let rawValue = input.value.replace(/[^0-9]/g, '');  // ✅ let 키워드 추가
	// 원래 숫자 값 유지
	input.dataset.rawValue = rawValue;
	// 천 단위 콤마 추가하여 표시
	input.value = rawValue ? Number(rawValue).toLocaleString() : '';
}
...
// Rating Selected
function sanchoSetRating(button) {
	// 모든 별 버튼을 가져옵니다.
	const ratingStarContainer = button.closest('.rating-star');
	const stars = ratingStarContainer.querySelectorAll('.rating-star .ui-btn');
	let isActive = true; // 클릭한 버튼까지 활성화하기 위한 플래그
	...
	const starInput = ratingStarContainer.querySelector('.rating-star .sancho-input');
	starInput.setAttribute('value', rating);
}
